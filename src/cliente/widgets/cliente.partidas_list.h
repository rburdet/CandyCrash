...
class ModelColumns : public Gtk::TreeModel::ColumnRecord
{
	public:
		ModelColumns()
		{ add(m_col_nivel); add(m_col_id); add(m_col_nombre); 
			add(m_col_conectados); add(m_col_percentage);}
...
class ListaPartidas : public Gtk::TreeView {
	public:
		ListaPartidas(){
			...
			//this->append_column_numeric("Id", m_Columns.m_col_id, "%010d"); 
		...
		void addRow(unsigned int m_col_nivel, unsigned long m_col_id, 
				std::string& m_col_nombre, short m_col_conectados, int m_col_percentage){
